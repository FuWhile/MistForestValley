...
const fetchShopItems = async () => {
  const response = await fetch("/api/npc/shop");
...
const fetchFarmers = async () => {
  const response = await fetch("/api/npc/farmers");
...
const buyItem = async () => {
  ...
  await fetch("/api/npc/buy", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ item_id: selectedItem.value.id }),
  });
  alert(`购买 ${selectedItem.value.name} 成功！`);
  fetchShopItems();
};
